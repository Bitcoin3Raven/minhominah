-- Supabase 보안 경고 해결을 위한 SQL 스크립트
-- 모든 함수에 search_path 설정 추가 및 보안 개선
-- Supabase 대시보드의 SQL Editor에서 실행하세요

-- 1. auto_empty_trash 함수 수정
CREATE OR REPLACE FUNCTION public.auto_empty_trash()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
-- 3. update_user_role 함수 수정 (보안 강화)
...
-- 7. cleanup_expired_invitations 함수 수정
...
-- 8. update_comment_updated_at 함수 수정
...
-- 9. generate_invitation_code 함수 수정
...
-- 10. generate_invitation_token 함수 수정
...
-- 12. add_creator_as_parent 함수 수정
CREATE OR REPLACE FUNCTION public.add_creator_as_parent()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
-- 14. accept_family_invitation 함수 수정
...
-- 15. create_default_notification_settings 함수 수정
...
-- 추가 보안 권장사항:
-- 1. 손상된 비밀번호 방지를 위해 Supabase Auth의 기본 설정 사용
-- 2. 정기적으로 만료된 초대 정리를 위한 크론 작업 설정
-- 3. 활동 로그 정기 백업 및 아카이빙 고려