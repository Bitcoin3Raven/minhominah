...
-- 2. 모든 스키마에서 get_comments_tree 함수 검색
SELECT 
...
-- 3. public 스키마에 함수가 있는지 확인
SELECT EXISTS (
    SELECT 1 
    FROM pg_proc p
    JOIN pg_namespace n ON p.pronamespace = n.oid
    WHERE n.nspname = 'public' AND p.proname = 'get_comments_tree'
) as function_exists_in_public;

-- 4. 함수 삭제 (있다면)
DROP FUNCTION IF EXISTS public.get_comments_tree(UUID);

-- 5. 함수 재생성 (public 스키마에 명시적으로)
...
-- 7. 함수 생성 확인
SELECT 
    'get_comments_tree 함수 생성 상태:' as info,
    EXISTS (
        SELECT 1 
        FROM pg_proc p
        JOIN pg_namespace n ON p.pronamespace = n.oid
...
ALTER PUBLICATION supabase_realtime DROP TABLE IF EXISTS public.comment_notifications;
